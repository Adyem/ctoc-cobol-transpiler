...
FT_TEST(test_parser_accepts_unary_minus_expression)
{
    const char *source;
    t_parser parser;
    t_ast_node *program;
    ...
    t_ast_node *expression;
    t_ast_node *operator_node;
    t_ast_node *operand_node;

    source = "IDENTIFICATION DIVISION.\n"
        "PROGRAM-ID. UNARY-MINUS.\n"
        "ENVIRONMENT DIVISION.\n"
        "DATA DIVISION.\n"
        "WORKING-STORAGE SECTION.\n"
        "01 SOURCE PIC 9(4).\n"
    ...
    expression = ast_node_get_child(assignment, 0);
    if (!expression || expression->kind != AST_NODE_UNARY_EXPRESSION)
    {
        ast_node_destroy(program);
        return (FT_FAILURE);
    }
    ...
    operand_node = ast_node_get_child(expression, 1);
    if (!operand_node || operand_node->kind != AST_NODE_IDENTIFIER)
    {
        ast_node_destroy(program);
        return (FT_FAILURE);
    }
...
FT_TEST(test_parser_accepts_unary_plus_expression)
{
    const char *source;
    t_parser parser;
    t_ast_node *program;
    ...
    if (operator_node->token.kind != LEXER_TOKEN_PLUS)
    {
        ast_node_destroy(program);
        return (FT_FAILURE);
    }
...
FT_TEST(test_parser_accepts_unary_abs_expression)
{
    const char *source;
    t_parser parser;
    t_ast_node *program;
    ...
    if (operator_node->token.kind != LEXER_TOKEN_KEYWORD_ABS)
    {
        ast_node_destroy(program);
        return (FT_FAILURE);
    }
