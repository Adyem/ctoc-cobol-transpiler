...
static t_ast_node *semantics_build_program_with_storage(const char *storage_name)
{
    t_ast_node *program;
...
FT_TEST(test_semantics_accepts_declared_move)
{
    ...
    if (transpiler_context_init(&context) != FT_SUCCESS)
        return (FT_FAILURE);
    program = semantics_build_program_with_storage("TARGET");
    if (!program)
    {
...
FT_TEST(test_semantics_rejects_undeclared_identifier)
{
    ...
    if (transpiler_context_init(&context) != FT_SUCCESS)
        return (FT_FAILURE);
    program = semantics_build_program_with_storage("PRIMARY");
    if (!program)
    {
        transpiler_context_dispose(&context);
        return (FT_FAILURE);
...
FT_TEST(test_semantics_detects_duplicate_data_item)
{
    ...
    if (transpiler_context_init(&context) != FT_SUCCESS)
        return (FT_FAILURE);
    program = semantics_build_program_with_storage("SHARED");
    if (!program)
    {
        transpiler_context_dispose(&context);
        return (FT_FAILURE);
