...
FT_TEST(test_standard_library_atoi_string_generates_expected_text)
{
    char *program_text;
    const char *expected_text;
    int status;
    ...
    if (test_expect_success(transpiler_standard_library_generate_atoi_string(&program_text),
            "atoi string generator should succeed") != FT_SUCCESS)
    {
        if (program_text)
            cma_free(program_text);
        return (FT_FAILURE);
    ...
    status = test_expect_cstring_equal(program_text, expected_text,
        "atoi string generator should emit expected COBOL subprogram");
    if (program_text)
        cma_free(program_text);
    if (status != FT_SUCCESS)
        return (FT_FAILURE);
...
FT_TEST(test_standard_library_atoi_converts_signed_value)
{
    ...
    driver_text =
        ...
        "       DATA DIVISION.\n"
        "       WORKING-STORAGE SECTION.\n"
        "       01 INPUT-TEXT PIC X(16) VALUE \"   -12345   \".\n"
        "       01 INPUT-LENGTH PIC S9(9) COMP-5 VALUE +12.\n"
        "       01 OUTPUT-RESULT PIC S9(9) VALUE 000000000.\n"
        "       01 OUTPUT-STATUS PIC 9 VALUE 0.\n"
        "       01 RESULT-DISPLAY PIC -9(9).\n"
        "       PROCEDURE DIVISION.\n"
        "           CALL 'CBLC-ATOI' USING BY REFERENCE INPUT-TEXT\n"
...
FT_TEST(test_standard_library_atoi_rejects_invalid_input)
{
    ...
    driver_text =
        ...
        "       DATA DIVISION.\n"
        "       WORKING-STORAGE SECTION.\n"
        "       01 INPUT-TEXT PIC X(8) VALUE \"123A5\".\n"
        "       01 INPUT-LENGTH PIC S9(9) COMP-5 VALUE +5.\n"
        "       01 OUTPUT-RESULT PIC S9(9) VALUE 000000000.\n"
        "       01 OUTPUT-STATUS PIC 9 VALUE 0.\n"
        "       01 RESULT-DISPLAY PIC -9(9).\n"
        "       PROCEDURE DIVISION.\n"
