...
FT_TEST(test_standard_library_memcmp_string_generates_expected_text)
{
    char *program_text;
    const char *expected_text;
    int status;
    ...
    expected_text =
        ...
        "       01 LNK-RESULT PIC S9(9).\n"
        "       PROCEDURE DIVISION USING BY REFERENCE LNK-FIRST\n"
        "           BY REFERENCE LNK-SECOND BY VALUE LNK-COUNT\n"
        "           BY REFERENCE LNK-RESULT.\n"
        "       MAIN.\n"
        "           MOVE 0 TO LNK-RESULT.\n"
        ...
        "           END-IF.\n"
        "           GOBACK.\n"
        "       END PROGRAM CBLC-MEMCMP-STRING.\n";
    if (test_expect_success(transpiler_standard_library_generate_memcmp_string(&program_text),
            "memcmp string generator should succeed") != FT_SUCCESS)
    {
        if (program_text)
            cma_free(program_text);
        return (FT_FAILURE);
    ...
    status = test_expect_cstring_equal(program_text, expected_text,
        "memcmp string generator should emit expected COBOL subprogram");
    if (program_text)
        cma_free(program_text);
    if (status != FT_SUCCESS)
        return (FT_FAILURE);
...
FT_TEST(test_standard_library_memcmp_executes)
{
    ...
    driver_text =
        ...
        "               BY REFERENCE FIRST-LENGTH BY REFERENCE SECOND-BUFFER\n"
        "               BY REFERENCE SECOND-LENGTH BY REFERENCE REQUEST-COUNT\n"
        "               BY REFERENCE RESULT.\n"
        "           IF RESULT < 0\n"
        "               DISPLAY \"LESS\"\n"
        "           END-IF.\n"
