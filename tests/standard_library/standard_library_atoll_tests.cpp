...
FT_TEST(test_standard_library_atoll_string_generates_expected_text)
{
    char *program_text;
    const char *expected_text;
    int status;
    ...
    expected_text =
        ...
        "           MOVE ACCUMULATOR TO LNK-RESULT.\n"
        "           GOBACK.\n"
        "       END PROGRAM CBLC-ATOLL-STRING.\n";
    if (test_expect_success(transpiler_standard_library_generate_atoll_string(&program_text),
            "atoll string generator should succeed") != FT_SUCCESS)
    {
        if (program_text)
            cma_free(program_text);
        return (FT_FAILURE);
    ...
    status = test_expect_cstring_equal(program_text, expected_text,
        "atoll string generator should emit expected COBOL subprogram");
    if (program_text)
        cma_free(program_text);
    if (status != FT_SUCCESS)
        return (FT_FAILURE);
...
FT_TEST(test_standard_library_atoll_converts_extended_value)
{
    ...
    driver_text =
        ...
        "       DATA DIVISION.\n"
        "       WORKING-STORAGE SECTION.\n"
        "       01 INPUT-TEXT PIC X(64) VALUE \"-123456789012345678901234567890\".\n"
        "       01 INPUT-LENGTH PIC S9(9) COMP-5 VALUE +31.\n"
        "       01 OUTPUT-RESULT PIC S9(36) VALUE 000000000000000000000000000000000000.\n"
        "       01 OUTPUT-STATUS PIC 9 VALUE 0.\n"
        "       01 RESULT-DISPLAY PIC -9(36).\n"
        "       PROCEDURE DIVISION.\n"
        "           CALL 'CBLC-ATOLL' USING BY REFERENCE INPUT-TEXT\n"
        "               BY REFERENCE INPUT-LENGTH BY REFERENCE OUTPUT-RESULT\n"
        "               BY REFERENCE OUTPUT-STATUS.\n"
        "           MOVE OUTPUT-RESULT TO RESULT-DISPLAY.\n"
...
FT_TEST(test_standard_library_atoll_rejects_trailing_garbage)
{
    ...
    driver_text =
        ...
        "       DATA DIVISION.\n"
        "       WORKING-STORAGE SECTION.\n"
        "       01 INPUT-TEXT PIC X(64) VALUE \"123456XYZ\".\n"
        "       01 INPUT-LENGTH PIC S9(9) COMP-5 VALUE +9.\n"
        "       01 OUTPUT-RESULT PIC S9(36) VALUE 000000000000000000000000000000000000.\n"
        "       01 OUTPUT-STATUS PIC 9 VALUE 0.\n"
        "       01 RESULT-DISPLAY PIC -9(36).\n"
        "       PROCEDURE DIVISION.\n"
